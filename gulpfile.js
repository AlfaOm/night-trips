...
// вместо "dist" по пременной ниже будет создаваться папка
// с конечными файлами по имени проекта
...
// взято https://fls.guru/gulp.html
function fontsStyle(params) {
  let file_content = fileSystem.readFileSync(source_folder + '/sass/fonts.scss');
  if (file_content == '') {
    fileSystem.writeFile(source_folder + '/sass/fonts.scss', '', cb);
      ...
      if (items) {
        ...
        for (let i = 0; i < items.length; i++) {
          ...
          if (c_fontname != fontname) {
            fileSystem.appendFile(source_folder + '/sass/fonts.scss', '@include font("' + fontname + '", "' + fontname + '", "400", "normal");\r\n', cb);
          }
          c_fontname = fontname;
        }
      }
